...
export default async function Gallery(){
  ...
  const allImages = uploadedImages.map((image) => ({ title: image.title, image: image.url, position: "center" }));
  return <main>
  ...
  <section className="section"><div className="container"><div className="section-title"><span>REAL PRODUCT PHOTOS</span><h2>PACK INDIA <span className="orange">CATALOGUE</span></h2><p>Browse the actual Pack India product photographs supplied for the website. The catalogue images cover paper products, films and pouches, tapes, strapping products and packaging machines.</p></div>
    {allImages.length ? <div className="catalogue-gallery">{allImages.map((product, index)=><article className="catalogue-card" key={`${product.title}-${index}`}><div className="catalogue-image"><img src={product.image} alt={product.title} loading="lazy" style={{objectPosition: product.position}}/></div><div className="catalogue-card-body"><span>PACK INDIA PRODUCT</span><h3>{product.title}</h3></div></article>)}</div> : <p className="gallery-empty">Our Gallery is being updated. Please check back soon.</p>}
  </div></section>
